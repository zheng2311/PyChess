...
# Creates a chess piece class that shows what team a piece is on, what type of piece it is and whether or not it can
# be killed by another selected piece.
class Piece:
...
# Basically, check black and white pawns separately and checks the square above them. If its free that space gets an
# "x" and if it is occupied by a piece of the opposite team then that piece becomes killable.
...
# This just checks a 3x3 tile surrounding the king. Empty spots get an "x" and pieces of the opposite team become
# killable.
...
# This creates 4 lists for up, down, left and right and checks all those spaces for pieces of the opposite team. The
# list comprehension is pretty long so if you don't get it just msg me.
...
# Same as the rook but this time it creates 4 lists for the diagonal directions and so the list comprehension is a
# little bit trickier.
...
# applies the rook moves to the board then the bishop moves because a queen is basically a rook and bishop in the
# same position.
...
# Checks a 5x5 grid around the piece and uses pythagoras to see if if a move is valid. Valid moves will be a distance
# of sqrt(5) from centre
...
# This is creating the window that we are playing on, it takes a tuple argument which is the dimensions of the window
# so in this case 800 x 800px
...
# This is creating the nodes that's are on the board(so the chess tiles)
# I've put them into a 2d array which is identical to the dimensions of the chessboard
...
# this takes in 2 co-ordinate parameters which you can get as the position of the piece and then the position of the
# node it is moving to you can get those co-ordinates using my old function for swap
